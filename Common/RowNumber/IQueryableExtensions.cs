...
namespace Caspian.Common.RowNumber
{
    public static class IQueryableExtensions
    {
        ...
        private static IQueryable CreateSelectManyExpr(this IQueryable source)
        {
            ...
            var rowNumberExpr = CreateRowNumberExpression(source, t, u);
        ...
        private static Expression CreateRowNumberExpression(this IQueryable query, ParameterExpression parameterExpr, ParameterExpression innerParameter)
        {
            ...
            if (desc.Count == 0)
            {
                ...
                var pkey = query.ElementType.GetPrimaryKey();
                var parameter = Expression.Parameter(query.ElementType, "t");
                var expr = Expression.Property(parameter, pkey);
                var lamda = Expression.Lambda(expr, parameter);
                query = query.OrderBy(lamda);
            }
        ...
        public static int? GetRowNumber(this IQueryable query, int id)
        {
            ...
            var rowNumber = selectManyQuery.Where_(whereExpr).FirstOrDefault();
            if (rowNumber == null)
                return null;
