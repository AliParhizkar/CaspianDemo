...
(function ($) {
    ...
    var $t = $.telerik = {
        ...
        bindControl: function (control, options, controlType) {
            ...
            switch (controlType) {
                ...
                case 7:
                    ...
                    break;
                case 8:
                    if (!$(control).data('tComboBox')) 
                        $(control).tComboBox(options);
                    $(control).data('tComboBox').updateState(options);
                    break;
            }
        },
        ...
        showConfirm: async function (message) {
            var $main = $('<div class="t-confirm"></div>');
            $main.append('<div class="t-title"></div>')
                .append('<span>' + message + '</span>');
            $main.append('<div class="t-content"></div>')
                .append('<button class="c-yes">بله</button><button class="c-no">نه</button>').appendTo('body');
            $main.find('.c-yes').click(function () {
                $main.remove();
                $.telerik.resolve(true);
            });
            $main.find('.c-no').click(function () {
                $main.remove();
                $.telerik.resolve(false);
            });
            let promise = new Promise(function (resolve, reject) {
                $.telerik.resolve = resolve;
            });
        ...
        bindLookup: function (input, searchForm, options) {
            ...
            $(searchForm).appendTo('body').css('top', $(input).position().top + 40);
        ...
        bindLookupSearchValue: function (dotnetHelper, input) {
            ...
            //    $(input).data('tTextBox').searchForm.open();
        ...
        bindLookupValue: function (dotnetHelper, input) {
            //$(input).unbind('changeValue.__valueChanged__');
            //$(input).bind('changeValue.__valueChanged__', function () {
            //    let value = $(input).data('tTextBox').value();
            //    dotnetHelper.invokeMethodAsync('UpdateSearchValue', value);
            //});
        },
